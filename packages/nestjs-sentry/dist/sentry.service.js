...
const severity_enum_1 = require("./severity.enum");
let SentryService = exports.SentryService = SentryService_1 = class SentryService extends common_1.ConsoleLogger {
    constructor(opts) {
        super();
        this.opts = opts;
                ...
                new Sentry.Integrations.OnUncaughtException({
                    onFatalError: (err) => __awaiter(this, void 0, void 0, function* () {
                        ...
                        if (err.name === 'SentryError') {
                            ...
                            (_a = Sentry.getCurrentHub().getClient()) === null || _a === void 0 ? void 0 : _a.captureException(err);
                            process.exit(1);
                        }
                    })
                }),
        ...
        try {
            ...
            asBreadcrumb
                ? Sentry.addBreadcrumb({
                    message,
                    level: severity_enum_1.Severity.Log,
                    data: {
                        context
                    }
                })
                : Sentry.captureMessage(message, severity_enum_1.Severity.Log);
        }
        catch (err) { }
    }
    error(message, trace, context) {
        ...
        try {
            ...
            Sentry.captureMessage(message, severity_enum_1.Severity.Error);
        }
        catch (err) { }
    }
    warn(message, context, asBreadcrumb) {
        ...
        try {
            ...
            asBreadcrumb
                ? Sentry.addBreadcrumb({
                    message,
                    level: severity_enum_1.Severity.Warning,
                    data: {
                        context
                    }
                })
                : Sentry.captureMessage(message, severity_enum_1.Severity.Warning);
        }
        catch (err) { }
    }
    debug(message, context, asBreadcrumb) {
        ...
        try {
            ...
            asBreadcrumb
                ? Sentry.addBreadcrumb({
                    message,
                    level: severity_enum_1.Severity.Debug,
                    data: {
                        context
                    }
                })
                : Sentry.captureMessage(message, severity_enum_1.Severity.Debug);
        }
        catch (err) { }
    }
    verbose(message, context, asBreadcrumb) {
        ...
        try {
            ...
            asBreadcrumb
                ? Sentry.addBreadcrumb({
                    message,
                    level: severity_enum_1.Severity.Info,
                    data: {
                        context
                    }
                })
                : Sentry.captureMessage(message, severity_enum_1.Severity.Info);
        }
        catch (err) { }
    }
    instance() {
